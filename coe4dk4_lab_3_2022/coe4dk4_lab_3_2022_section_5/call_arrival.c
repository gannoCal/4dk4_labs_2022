...
call_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  new_call->queue_time = -1;
  new_call->call_duration =  exponential_generator((double)sim_data->mean_call_time);
  ...
  if((free_channel = get_free_channel(simulation_run)) != NULL) {
    ...
    sim_data->non_queued_call_count++;
    ...
    //sim_data->blocked_call_count++;
      sim_data->queued_call_count++;
      new_call->hangup_time = exponential_generator(sim_data->mean_hangup_time);
...
Channel_Ptr get_free_channel(Simulation_Run_Ptr simulation_run)
{
  ...
  for (i=0; i< sim_data->trunk_count; i++) {
