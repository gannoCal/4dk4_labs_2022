...
end_call_on_channel_event(Simulation_Run_Ptr simulation_run, void * c_ptr)
{
  ...
  //capture queing time
  if (this_call->queue_time < sim_data->t_val) {
	  sim_data->wait_less_t++;
  }

  output_progress_msg_to_screen(simulation_run);

  int call_found = 0;
  while (fifoqueue_size(sim_data->buffer) > 0 && !call_found) {
	  next_call = (Call_Ptr)fifoqueue_get(sim_data->buffer);
	  next_call->queue_time = now - next_call->arrive_time;
	  if (next_call->queue_time < next_call->hangup_time || 1==1) {
		  server_put(channel, (void*)next_call);
		  next_call->channel = channel;

		  
		  sim_data->queuing_time_accum += next_call->queue_time;
		  sim_data->queuing_time_accum_no_hangup += next_call->queue_time;
		  sim_data->non_hangup_count++;
		  ...
		  sim_data->hangup_count++;
		  sim_data->queuing_time_accum += next_call->hangup_time;
