...
int main(void)
{
  ...
  long int number_of_channels_data_points = 50;
  ...
  //calc arrival rates
  double arrival_rates[100001];
  double arrival_rate_min =0.001/3;
  int arrival_rate_data_points = 100000;
  double arrival_rate_max = 10;
  ...
  double pb_results_p01[100];
  double pb_results_p01_pb[100];
  ...
  while ((random_seed = RANDOM_SEEDS[j++]) != 0) {
      ...
      while ((number_of_channels[l++]) != -1) {
          k = 0;
          found_p01 = 0;
        while ((arrival_rates[k++]) != -1 && !found_p01) {
              ...
              if (theoretical_erlang_B >= 0.01 && !found_p01) {
                  found_p01 = 1; 
                  pb_results_p01[l - 1] = offered_load;
                  pb_results_p01_pb[l - 1] = theoretical_erlang_B;
              }
  ...
  i = 0;
   index = 0;
    while (number_of_channels[i] != -1) {
        ...
        index += snprintf(&str[index], OUT_STR_SIZE - index, "%.15f,", pb_results_p01[i]);

        int load_p01 = pb_results_p01[i];
        index += snprintf(&str[index], OUT_STR_SIZE - index, "%.15f,", pb_results_p01_pb[i]);
