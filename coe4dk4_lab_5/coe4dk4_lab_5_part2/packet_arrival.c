...
packet_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
   ...
   * the buffer.
   */
  int random_station_id = (int)floor(uniform_generator() * 2.0);
  if (random_station_id == 1) {
	  new_packet->service_time =0.01;
	  new_packet->source_id = 1;
	  if (server_state(data->good_phone_server) == BUSY) {
		  fifoqueue_put(data->good_phone_buffer, (void*)new_packet);
	  }
	  else {
		  start_transmission_to_base(simulation_run, new_packet, data->good_phone_server);
	  }
  }
  else {
	  new_packet->service_time = 0.1;
	  new_packet->source_id = 0;
	  if (server_state(data->bad_phone_server) == BUSY) {
		  fifoqueue_put(data->bad_phone_buffer, (void*)new_packet);
	  }
	  else {
		  start_transmission_to_base(simulation_run, new_packet, data->bad_phone_server);
	  }
  }
