...
output_progress_msg_to_screen(Simulation_Run_Ptr simulation_run)
{
  ...
  if((data->blip_counter >= BLIPRATE)
    ...
    printf("Sim:%3.0f%%, ", 100 * (double)data->run_count / data->runs_required);
    printf("Run count:%d, ", data->run_count);
    printf("%3.0f%% ", percentage_done);

    /*printf("Successfully Xmtted Pkts  = %ld, packets recieved at base = %ld (Arrived Pkts = %ld) \r",
	   data->number_of_packets_processed, data->number_of_packets_processed_base ,data->arrival_count);*/
...
output_results(Simulation_Run_Ptr simulation_run)
{
  ...
  printf("Mean Delay Good (msec) = %.2f \n",
	 1e3*data->accumulated_delay_good/data->number_of_packets_processed_good);

  printf("Mean Delay Bad (msec) = %.2f \n",
      1e3 * data->accumulated_delay_bad / data->number_of_packets_processed_bad);
  
  printf("bad size = %0d \n",
      data->max_bad_buffer);
  printf("good size = %0d \n",
      data->max_good_buffer);
  printf("\n");
}
