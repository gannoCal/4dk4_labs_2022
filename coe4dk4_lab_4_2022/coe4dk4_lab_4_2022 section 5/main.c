...
main(void)
{
  ...
  double* mean_delay = malloc(mean_backoff_duration_data_points * number_of_stations_data_points * arrival_rate_data_points * sizeof(double));
  double* throughput = malloc(mean_backoff_duration_data_points * number_of_stations_data_points * arrival_rate_data_points * sizeof(double));
  double* throughput2 = malloc(mean_backoff_duration_data_points * number_of_stations_data_points * arrival_rate_data_points * sizeof(double));
  double* G_rate = malloc(mean_backoff_duration_data_points * number_of_stations_data_points * arrival_rate_data_points * sizeof(double));
      ...
      while ((mean_backoff_duration_arr[k++]) != -1) {
          ...
          while ((number_of_stations_arr[l++]) != -1) {
              ...
              while ((arrival_rates[h++]) != -1) {
                  ...
                  data.epsilon = MEAN_PACKET_DURATION * 0.01;
                  ...
                  while (data.number_of_packets_processed < RUNLENGTH && (data.number_of_packets_processed == 0 || ((data.number_of_packets_processed > 0) && ((data.accumulated_delay / data.number_of_packets_processed) < 7e5)))) {
