...
main(void)
{
  ...
  int arrival_rate_data_points = 200;
  double arrival_rate_max = 20;
      ...
      while ((mean_backoff_duration_arr[k++]) != -1) {
          ...
          while ((number_of_stations_arr[l++]) != -1) {
              ...
              while ((arrival_rates[h++]) != -1) {
                  ...
                  while (data.number_of_packets_processed < RUNLENGTH && (data.number_of_packets_processed == 0 || ((data.number_of_packets_processed > 0) && ((data.accumulated_delay / data.number_of_packets_processed) < 101e6)))) {
