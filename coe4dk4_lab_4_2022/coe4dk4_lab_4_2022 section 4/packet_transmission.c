...
transmission_start_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  if (this_packet->station_id != 0) data->attempted_transmissions++;
  (data->stations + this_packet->station_id)->attempted_transmissions++;
...
transmission_end_event(Simulation_Run_Ptr simulation_run, void * packet)
{
  ...
  if(get_channel_state(channel) == SUCCESS) {
    ...
    /* Collect statistics. */
    if (this_packet->station_id != 0) data->number_of_packets_processed++;
    ...
    if (this_packet->station_id != 0) data->accumulated_delay += now - this_packet->arrive_time;

    if (this_packet->station_id != 0) data->successful_transmit_time += this_packet->service_time;
    (data->stations + this_packet->station_id)->successful_transmit_time += this_packet->service_time;
