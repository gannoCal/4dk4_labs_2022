...
output_blip_to_screen(Simulation_Run_Ptr simulation_run)
{
  ...
  if((data->blip_counter >= BLIPRATE)
    ...
    fprintf(fp, "Sim:%3.0f%%, ", 100 * (double)data->run_count / data->runs_required);
    fprintf(fp, "Run count:%d, ", data->run_count);
    fprintf(fp, "Run:%3.0f%% ", percentagedone);
    fprintf(fp, "Runtime:%3.0f ", simulation_run->clock->time);

    fprintf(fp, "Successfully Xmtted Pkts  = %ld (Arrived Pkts = %ld) \r",
        data->number_of_packets_processed,
        data->arrival_count);
    fprintf(fp, "\n");
    fflush(fp);*/
