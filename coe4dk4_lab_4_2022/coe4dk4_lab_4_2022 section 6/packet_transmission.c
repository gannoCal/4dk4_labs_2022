				  ...
				  void * packet) 
{
  ...
  double slot_time = ((data->mean_data_duration/data->reservation_ratio) + 2 * data->epsilon);

  double div_val = event_time / slot_time;
  double ciel_val = CEILING(div_val);
  double next_slot_time = slot_time * ciel_val + data->epsilon;

  if (next_slot_time < 0) {

      next_slot_time = 10;
      /*printf("Run:%3.0f%% ", slot_time);
      printf("Run:%3.0f%% ", event_time);
      printf("Run:%3.0f%% ", ciel_val);
      printf("Run:%3.0f%% ", div_val);*/
  }

  return simulation_run_schedule_event(simulation_run, event, next_slot_time);
}
...
transmission_start_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  data = (Simulation_Run_Data_Ptr) simulation_run_data(simulation_run);
  channel = data->reservation_channel;
  data->attempted_transmissions++;
...
data_transmission_start_event(Simulation_Run_Ptr simulation_run, void* ptr)
{
    ...
    this_packet->service_time = exponential_generator(data->mean_data_duration);
...
{
  Packet_Ptr this_packet, next_packet, data_packet;
  Buffer_Ptr buffer;
  ...
  Buffer_Ptr data_buffer = data->data_buffer;
  ...
  if(get_channel_state(channel) == SUCCESS) {
    ...
    //data->successful_transmit_time += this_packet->service_time;
    //data->successful_transmit_time += 1;

    //output_blip_to_screen(simulation_run);

    //schedule a new data transmission
    schedule_data_transmission_start_event(simulation_run,
        now,
        (void*)fifoqueue_get(buffer));
